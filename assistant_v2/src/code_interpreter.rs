...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // poll the status of run until it's in a terminal state
    loop {
        match run.status {
            RunStatus::Completed => {
                let messages = client
                ...
                for message_obj in messages.data {
                    for message_content in message_obj.content {
                        match message_content {
                            MessageContent::Text(text) => {
                                let text_data = text.text;
                                println!("{}", text_data.value);
                                for annotation in text_data.annotations {
                                    match annotation {
...
mod tests {
    ...
    #[tokio::test]
    async fn build_code_interpreter_request() {
